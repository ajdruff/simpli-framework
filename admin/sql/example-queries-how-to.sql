...
-- Altering Tables
--------------
-- Tip: Use Toad to generate existing sql creation script and then use the templates below

-- change column name
ALTER TABLE `nstock_stats_dn_impress_history2` change `domain2_id` `domain3_id` int(11)

-- alter column
ALTER TABLE `nstock_stats_dn_impress_history2` MODIFY COLUMN `domain3_id` int(12)

-- add unique key 
CREATE UNIQUE INDEX `domain_date_hour` ON `nstock_stats_dn_impress_history2`(`domain_id`,`date`,`hour`);
...
(name, tld, bin, bid, price, currency, time_added, featured, seller, approved, time_lastupdated, time_approved, reg_available, time_list_start, time_list_stop, list_status, not_listed_reason, price_note, source, added_by) 
VALUES (
'test',   -- name
'com', -- bin
'y',  -- bin

...
DROP PROCEDURE IF EXISTS `simpli_wp_dev`.`nstock_activate_approved_listings`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `nstock_activate_approved_listings`(LIST_TIME_IN_HOURS INT)
BEGIN

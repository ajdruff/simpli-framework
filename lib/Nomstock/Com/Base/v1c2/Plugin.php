...
//wp_localize_script($this->getSlug() . '_' . 'localVars', $this->getSlug(), $this->_local_vars );
    ...
    public function init() {
        ...
        /*
         * Add extra headers so we can read Addon file data
         * using the WordPress API
         * Ref:
         */
        add_filter('extra_simpli_headers', array($this, 'hookAddExtraWpHeaders'));
...
//        $filename = $filename . '.php'; # Admin.php
    ...
    public function loadAddons() {
        ...
        $addon_files = $tools->getGlobFiles(
                $this->getAddonsDirectory() //where we are looking for it
                , 'Addon.php' //the file we are looking for
                , true //recursive
                , array() //array of files that are carried through if called recursively. always set to empty array.
                , '#.*\/temp.*#' //exclusion pattern . will ignore all directories starting with /temp . This allows us to ignore any recently uploaded addons that are still stuck in the /temp directory, without giving us 'already declared' errors.
        );
...
//        /*
//         * Underscore - needed for utilities
//         */
//
//        wp_enqueue_script('underscore');
         ...
         * 
         * 2/20 - removed this, i don't think we need it
         * 
         * 
         */
        ...
        $ver = null;
        $in_footer = false;
       // wp_enqueue_script($handle, $src, $deps, $ver, $in_footer); // removed for optimization
        ...
        $vars = array(
            'plugin' => array(
                ...
                , 'query_var_action' => $this->QUERY_VAR . '_action'
                , 'name' => $this->getName()
                , 'url' => $this->getUrl()
                , 'version' => $this->getVersion()
                , 'directory' => $this->getDirectory()
                , 'debug' => $this->DEBUG && $this->debug()->isOn() //flag telling javascript whether debugging is on
    ...
    function hookPrintLocalVars() {
        $parts = $this->getSlugParts();
        $prefix = $parts[0];
        $suffix = $parts[1];
        $namespace = $prefix . '.' . $suffix . '.vars';
        $vars = json_encode($this->getLocalVars());
        /*
         * Pass all the variable sto
         */
        ?>
    ...
    protected $_addon_headers_added = false;

    /**
     * Hook - Add Extra Wp Headers
     *
     * Adds extra WordPress Hook extra_{$context}_headers
     * Ref:
     *
     * @param string $extra headers
     * @return void;
     */
    public function hookAddExtraWpHeaders($extra_headers) {

        /*
         * add headers if not already added.
         */
        if ($this->_addon_headers_added !== false) {
            return;
        }
        $this->_addon_headers_added = true; /* toggle flag so we know we know these dont need to be added any more */

        $extra_headers[] = 'Addon Name';
        $extra_headers[] = 'Addon URI';
        $extra_headers[] = 'Addon Slug';

        /**
         * When combined with the headers added by the
         * Framework.php, we get:
         *
         * Addon Name: Name Of The Plugin
         * Addon URI: http://URI_Of_Page_Describing_Plugin_and_Updates

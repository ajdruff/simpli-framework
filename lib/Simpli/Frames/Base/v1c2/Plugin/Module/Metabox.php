...
//      echo do_shortcode($template); //using buffer and do_shortcode is required to allow shortcodes to work within an included file, otherwise they dont render.
    ...
    protected function _createNonces() {
                ...
                /*
         * Block usage of this module's createNonces method by Non-Admin requests, since it will overwrite the values being written by Forms.php->_createNonces, causing the Nonce check to fail. 
         */
        
        if ( !is_admin()) { //check if admin to avoid nonces from being overwritten
            $this->debug()->log( 'Returning from _createNonces because form is not being created on an admin page, so it wont have access to these methods is not admin' );
            return;
}
        /*
...
//         */
//        $this->plugin()->setConfig('NONCE_DEFAULT_VALUE', wp_create_nonce($this->plugin()->getModule('Forms')->NONCE_DEFAULT_ACTION));
//        $this->debug()->log('Created Default Nonce using NONCE_DEFAULT_ACTION = ' . $this->plugin()->getModule('Forms')->NONCE_DEFAULT_ACTION);
        ...
        if ($this->plugin()->getModule('Forms')->NONCE_UNIQUE_ENABLED) {
            /*
...
//   $vars[$action_short_name . '_nonce_value'] = wp_create_nonce($action_long);
                ...
                $local_vars=$this->plugin()->getLocalVars($vars);
       ...
       $vars['forms']['nonce_values']=array_merge($local_vars['forms']['nonce_values'],$vars['forms']['nonce_values']);             
         ...
         */
        $vars['forms']['unique_action_nonces'] = $this->plugin()->getModule('Forms')->NONCE_UNIQUE_ENABLED;

        $vars['forms']['nonce_field_name'] = $this->plugin()->getModule('Forms')->NONCE_FIELD_NAME;
        $vars['forms']['nonce_values']['default'] = $this->plugin()->getModule('Forms')->NONCE_DEFAULT_VALUE;
    ...
    public function wpVerifyNonce($function_name = null) {
         ...
         */
        $nonce_value = $this->plugin()->tools()->getRequestVar($this->plugin()->getModule('Forms')->NONCE_FIELD_NAME);
                $this->debug()->logVar( '$_POST = ', $_POST);
                $this->debug()->logVar( '$_REQUEST = ', $_REQUEST);
        $this->debug()->logVar( '$this->plugin()->getModule(\'Forms\')->NONCE_FIELD_NAME = ', $this->plugin()->getModule('Forms')->NONCE_FIELD_NAME);
         ...
         *
         */
        if ($this->plugin()->getModule('Forms')->NONCE_UNIQUE_ENABLED && !is_null($function_name)) {
            /*
             ...
             */
            $nonce_action = $this->plugin()->getModule('Forms')->NONCE_DEFAULT_ACTION;

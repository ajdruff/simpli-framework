...
class Simpli_Frames_Base_v1c2_Plugin_PostType extends Simpli_Frames_Base_v1c2_Plugin_Menu {
    ...
    protected function _register_post_type($post_type, $args) {
        ...
        if (isset($result->errors)) {
            reset($result->errors);
            $first_error = key($result->errors);
            $this->debug()->logError($result->errors[$first_error][0]);
        } else {
            $this->debug()->log('Successfully Registered Post Type ' . $post_type);
        }
...
// Insert the post into the database
        ...
        exit(); // to help wp_redirect exit cleanly - should always be done
    }
...
// Insert the post into the database
    ...
    public function hookEnqueueFormScripts() {
        ...
        /*
         * We intententionally block the loading of scripts from the Menu base class by
         * overriding the hookEnqueueFormScripts base class method and not
         * loading anything here.
         *
         * This leaves the PostUserOptions module free to load scripts as needed
         * to support the post options forms.
         */

        $this->debug()->log('Menu base class script prevented from loading the
            menu scripts. PostUserOptions module should load any required scripts.
            ');
    }
